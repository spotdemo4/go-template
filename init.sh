...
old_slug=go-template
old_description='go template'
old_url=https://trev.zip/template/go
...
if $is_github; then
  ...
  if [[ -d .forgejo ]]; then
    ...
    if [[ $reply =~ ^[Yy]$ ]]; then
      ...
      replace_literal 'template/go' "$secondary_repo_path" .forgejo/renovate.json
  ...
  if [[ -f .forgejo/renovate.json ]]; then
    replace_literal 'template/go' "$repo_path" .forgejo/renovate.json
